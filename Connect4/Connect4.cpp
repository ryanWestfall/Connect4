...
//player class with pieces and name as varibles
class Player
{
public:
    string piece;
    ...
    Player(string p) : piece(p){}
    ~Player(){}
};

//Game class with the varibles as the pColumns and see when game is over.
class Game
{
    int pCol = 0;
    bool gameOver = false;
...
public:
    void titleDisplay(Player&, Player&);
...
void Game::titleDisplay(Player& one, Player& two)
{
    ...
    cout << setw(54) << "$ - "<< one.name <<"\n";
    cout << setw(54) << "@ - " << two.name <<"\n";
